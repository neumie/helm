...
for (const width of [320, 390, 1280]) {
	test(`source and status remain readable and reachable at ${width}px`, async ({ page }, testInfo) => {
		...
		await expect(okena).toContainText('Main Pi idle')
...
for (const viewport of [
	...
	test(`long detail identity ellipsizes in its single header row and wraps fully in Info at ${viewport.width}x${viewport.height}`, async ({
		...
		await expect(fullTitle).toHaveText(LONG_IDENTITY)
		const geometry = await fullTitle.evaluate((node, suffix) => {
...
for (const height of [420, 480, 560]) {
	test(`metadata-rich detail keeps controls, reading action, and composer reachable at 390x${height}`, async ({
		...
		await expectFullyContained(detail.getByRole('button', { name: 'Attachments, model and effort', exact: true }), {
			width: 390,
			height,
		})
...
test('compact native-rich long identity and source-unavailable details keep empty and typed composers contained', async ({
	...
	await openCurrentInfo(page)
	await expect(nativeDetail.locator('.remote-current-conversation dl')).toContainText('ProjectJVS')
...
test.describe('compact touch input', () => {
	...
	test('native-rich long identity scrolls metadata by touch and keeps menu rows hittable after growth', async ({
		...
		await expect(detail.getByRole('button', { name: 'Message delivery: Follow-up', exact: true })).toHaveText('')
...
async function openCurrentInfo(page: Page) {
...
async function closeCurrentInfo(page: Page) {
