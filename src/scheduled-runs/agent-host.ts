...
export function writeScheduledPrompt(runDir: string, prompt: string): string {
	if (Buffer.byteLength(prompt, 'utf8') > MAX_PROMPT_BYTES) throw new Error('Scheduled prompt exceeds private-file limit')
...
export function writeInvocationDescriptor(runDir: string, descriptor: ScheduledInvocationDescriptor): string {
	if (!isAbsolute(descriptor.cwd) || !isAbsolute(descriptor.promptPath)) throw new Error('Scheduled invocation paths must be absolute')
	if (!descriptor.invocation.command || !Array.isArray(descriptor.invocation.args)) throw new Error('Invalid scheduled invocation')
	const path = join(resolve(runDir), 'invocation.json')
	const encoded = JSON.stringify(descriptor)
	if (Buffer.byteLength(encoded, 'utf8') > MAX_DESCRIPTOR_BYTES) throw new Error('Scheduled invocation descriptor too large')
	mkdirSync(dirname(path), { recursive: true, mode: 0o700 })
...
export function readInvocationDescriptor(path: string): ScheduledInvocationDescriptor {
	...
	if (!value || !isAbsolute(value.cwd) || !isAbsolute(value.promptPath) || !value.invocation?.command || !Array.isArray(value.invocation.args)) {
...
export async function runScheduledAgentHost(descriptorPath: string, env = process.env): Promise<void> {
	...
	await waitForChild(descriptor.invocation.command, [...descriptor.invocation.args, prompt.toString('utf8')], descriptor.cwd, env)
...
if (process.argv[2] === '--scheduled-agent-host' && process.argv[3]) {
	runScheduledAgentHost(process.argv[3]).catch(error => {
		process.stderr.write(`scheduled agent host failed (${randomUUID()}): ${error instanceof Error ? error.message : String(error)}\n`)
		process.exitCode = 1
	})
}
