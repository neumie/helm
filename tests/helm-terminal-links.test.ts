...
test('terminal web links use the restricted external-open bridge instead of window.open', () => {
	assert.match(
		renderer,
		/new WebLinksAddon\(\(event, uri\) => \{\s*if \(!shouldOpenTerminalLink\(event\)\) return\s*event\.preventDefault\(\)\s*void helm\.external\.open\(uri\)/,
	)
...
test('terminal web links open only on Command + left click', () => {
	assert.equal(shouldOpenTerminalLink({ button: 0, metaKey: false }), false)
	assert.equal(shouldOpenTerminalLink({ button: 1, metaKey: true }), false)
	assert.equal(shouldOpenTerminalLink({ button: 2, metaKey: true }), false)
	assert.equal(shouldOpenTerminalLink({ button: 0, metaKey: true }), true)
})
