...
test('production IPC gates controller-backed transfer commands by sender and captured token', () => {
	...
	assert.match(main, /Promise\.resolve\(\{ status: 'rejected' as const, reason: 'stale-profile' \}\)/)
	assert.match(main, /terminalTransferMain\.move\(/)
	assert.match(main, /terminalTransferMain\.unregisterRendererCapability\(sessionId, capability\)/)
