...
import { openRemoteQuestion } from "./remote-answers.js";
...
function makeSessionFactory(config: {
	...
	pi: ExtensionAPI;
	remoteRef: { current: ReturnType<typeof openRemoteQuestion> | null };
}) {
	...
	return (
		...
		const remote = openRemoteQuestion(config.pi.events, typed, done);
		config.remoteRef.current = remote;
		const session = new Session({
			tui,
			theme,
			params: typed,
			itemsByTab,
			done: result => remote.complete(result),
			keybindings,
...
export function registerAskUserQuestionTool(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			const remoteRef: { current: ReturnType<typeof openRemoteQuestion> | null } = { current: null };
