...
| `--group-blue/purple/pink/red/orange/green/cyan` | palette in `theme-presets.ts` | derived at 15%/24% | User-selected terminal-group identity only (§3.18) |
...
- **Tab groups:** persisted named groups project independently in the strip and background popover. Terminals without a group stay in the ordinary tab/row flow with no heading, separator, or special label. A compact 11/600 section header precedes named groups only, as a quiet disclosure button. Each group owns a persisted user-selectable color from the seven-token `--group-*` palette: its label uses a 15% soft fill, strip members carry a 2px top marker, and Background members a 2px leading marker. The group menu's **Color…** submenu changes it; legacy/new groups receive a stable ID-derived default. Collapse is independently persisted per surface. Because `.tab` and `.bg-row` set explicit display modes, their `[hidden]` rule MUST explicitly set `display: none`; relying on the HTML user-agent rule makes collapse appear inert. A collapsed named section exposes exactly one existing terminal tab/row: the active member first, otherwise attention, then running, then canonical order. Non-representative members remain mounted but DOM-hidden; dragging the representative temporarily reveals its same-group peers. The representative keeps its real title and exact OSC activity/attention indicator. Right-clicking a tab opens its existing menu with Rename, Move to existing group, Move to new group, Move to profile, Move to background, and Close; a tab-group header opens Rename/Delete plus surface-specific bulk actions. Strip groups offer Move group to Background; background groups offer Open all (without changing background ownership), Restore all, and Close all. Delete returns members to the ordinary ungrouped flow. Group-name entry stays inside the same detached menu using the compact field grammar—never a new dialog. The Context Menu key / Shift+F10 opens a focused group header menu; menu Up/Down/Home/End, Esc, and click-outside follow §3.8. Bulk placement is explicit: ordinary membership changes preserve each terminal's foreground/background placement, while Restore/Move group intentionally change it. Empty groups have no projected header and are removed from persisted registry metadata.
