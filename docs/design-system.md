...
- **Item workspace action lives with the Item's destination rows, not in the identity hero.** It is a 28px external `ActionRow`: label **Workspace**, dynamic action value carrying the full effect (`Focus in Okena`, `Register worktree in Okena`, `Create Okena workspace from local branch`, `Fetch branch and create Okena workspace`, `Create branch and Okena workspace`, or `Focus main checkout in Okena`). There is no generic "Open in Okena" button and no adjacent explanatory caption—the button itself says what it will do. It is lifecycle-neutral and available in every status. Preview inspection may query Okena and git only on the single-Item detail route; list routes stay DB-only, and the POST recomputes the action for race safety. Success resolves the terminal from Okena's live pane layout, focuses the project, and best-effort raises its window. An already-open workspace receives exactly one BEL attention character so Okena paints its yellow border; no command or `ctrl_c` is sent. Toast **Focused in Okena** with the daemon's truthful focus/attention hint; a newly-opened workspace whose terminal is still becoming ready is still success, not a false retryable error. Actual workspace-open failures stay in Helm's retryable command error.
- Nav row (`.action-row-nav` — settings section lists and the Item detail's two remaining push rows): a tappable row whose **title is the content, not a label** — min-height **36** (36px pitch), title 13/400 `--text-0` sentence case, value 12 `--text-1` right-aligned single-line ellipsis, chevron `--text-2`. On Item detail, **Task** and **Plan documents** are peer reading destinations and stack with the 28px **Workspace** external action in one unlabeled `.card-flush` group; Plan documents never sits under ticket/status facts. Every nav row shows a current-state value with units and real state ("60s", "2 of 3 on", "default") — a blank cell next to a chevron reads as broken, a unit-less number gives no direction, and independent toggles never collapse into one fake on/off.
