...
- **Start loop semantics:** on a planned solve Item, this is the autonomous full-queue action—not “run one ticket.” It runs every currently agent-ready local/GitHub ticket in AFK mode, skips human-ready tickets, and uses the effective Agent + Model from Run setup (Item override, otherwise daemon default).
