...
- **Popover — editorial terminal list**: §3.8 panel (background `--chrome`, `--hairline`, radius `--radius-lg`, `--shadow-1`, z 40), fixed width 320 (viewport-clamped) and max-height `min(480px, 100vh - 48px)`, anchored `menu-end` under the button with a 5px offset so its border does not fuse to the header hairline; the generic menu padding is removed so row state stripes reach both panel edges, while the header and row content own their own insets. Rows scroll under the platform overlay scrollbar with NO reserved gutter while the header stays fixed, keeping left/right action spacing balanced. Its 12/600 header contains only “Background terminals”; the strip-control badge is the single count source, so the open panel never repeats it. One row per background terminal, park order.
  ...
  - Three explicit operations remain: clicking the title target **opens and focuses the terminal while it remains parked** (no tab button, count and persisted `parked` flag unchanged); quiet icon-only **⇥** (28px, tooltip/accessible name "Move to tabs and open") moves it to the strip end and opens it; quiet **×** closes through the grace path. The title target is ALSO a reciprocal direct-manipulation handle after the shared 5px threshold: dragging it onto the strip restores and activates that terminal at the chosen projected-unit slot. Sub-threshold release remains Open. Existing group membership is preserved—when that group already has strip members, the target snaps/highlights that group and inserts among its members; it never silently ungroups. Escape, blur, cancellation, invalid/outside drop, or a terminal disappearing mid-gesture leaves it parked. Both secondary controls use the shared `IconBtn` primitive (`createIconButton` is its plain-DOM adapter for this non-React renderer), never ad-hoc button chrome. Undo after close returns it to the background list, not to a tab.
