...
- **Run evidence and run setup are inline groups on Item detail** (the retired Work hub and its pushed pages are in the appendix), ordered per lifecycle state by `detail-state.ts` — decision content first: Activity (started/loop facts + the latest-first timeline, 5 events visible while running, 3 in settled states, then a "Show all" cue), Log (append-only output normalized to newest-first; the two newest non-empty messages remain visible at rest in `.log-well`, then "Show all" / "Show less" reveals the available history; failed mounts expanded directly beneath Failure), Solve input (§3.20 disclosure), and Run setup — a one-line effective-selection summary readable at rest (12 `--text-1`, "Claude Code · model · effort · Worktree") above a "Change setup" disclosure holding the four pickers. Planned items get a flush Plan group (status/spec/ticket facts + the Plan documents nav row); the planned-active hero already carries plan status, so the group omits its Status row there (no repeated fact).
...
The sanctioned way to keep heavy evidence in place without a subpage (`Disclosure` in `app/src/renderer/sidebar/ui.tsx`) — the promotion of the old ad-hoc `.detail-disclosure` + `useState` pattern. Solve input and Run setup use Disclosure because they have no useful collapsed content. Activity and Log deliberately keep the bare `.detail-disclosure` button: their first N rows/messages stay visible at rest, which Disclosure cannot model because closed children unmount.

- Shape: a quiet toggle **button** in the `.detail-disclosure` style (11/600 `--accent`, min-height 24) with `aria-expanded`/`aria-controls` and a sentence-case **verb pair** ("Show all" / "Show less", "Change setup" / "Hide setup"); revealed content follows in the group's flow.
