...
- **Strip control**: ghost button pinned at the strip's right edge (28px height, radius `--radius-md`), visible ONLY when the background count > 0 — the empty state is the hidden button, never an empty popover. Content: a native 14×14 layers SVG on its own 14-unit viewBox with a 1px non-scaling stroke (`--text-2`; never squeeze a 24px/2px icon into this slot—it renders soft/pixelated) + count badge in the §3.4 chip spec (neutral tone soft fill, 10/700 tabular). While a parked terminal is temporarily open in the pane, its display name appears between the glyph and count in 12/500 `--text-1`, truncated at `min(160px, 25vw)`; switching to a strip tab removes the name immediately. This names the otherwise tabless visible pane without changing Background ownership or count. Hover and open: `--fill-subtle` fill, `--text-0` glyph.
