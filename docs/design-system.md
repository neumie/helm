...
- **Item workspace row:** every Item detail's Work card ends with one 28px external-action row, `Workspace` → `Okena · <branch>` (or `Okena · create workspace` / `Okena · main checkout`) + ↗. It is lifecycle-neutral and available in every status: reuse/focus an existing Okena terminal, register an existing non-Okena worktree, or explicitly create + persist the Item worktree when none exists. Success focuses the Okena project and best-effort raises its window: toast **Opened in Okena** when raised, otherwise **Focused in Okena** with “Switch to Okena to view the workspace.” Failure stays in Helm's retryable command error. It never sends characters or `ctrl_c` into a reused terminal.
