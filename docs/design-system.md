...
The sanctioned way to keep heavy evidence in place without a subpage (`Disclosure` in `app/src/renderer/sidebar/ui.tsx`) — the promotion of the old ad-hoc `.detail-disclosure` + `useState` pattern. Solve input and Run setup use Disclosure because they have no useful collapsed content. Activity and Log put their disclosure controls in the Section-header trailing slot so the body contains evidence only. Activity renders **no body** until "Show history" is activated; expansion shows result summary + lifecycle events. Log is the deliberate preview exception: its two newest messages remain visible at rest.
