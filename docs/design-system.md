...
- Color follows activity: the bright head and immediate tail use `--accent`; inactive dots transition to neutral `--text-0` and fade toward the background. Neither variant uses glow.
- **Progress variant:** motion is clockwise: top-left → top-right → middle-right → bottom-right → bottom-left → middle-left. One dot advances every `--motion-state` interval (140ms), one 840ms cycle; color and opacity interpolate from the active head into the neutral tail.
