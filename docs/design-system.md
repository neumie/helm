...
- Height **64**, square full-width hover/active stripe (`border-radius: 0`) with horizontal padding 16 owned by the row; the scroll container has no horizontal inset, so the state fill reaches both usable pane edges while text remains on the 16px rhythm. **No separators** — the 64px pitch and line spacing carry the rhythm; no card border, no shadow. The persistent scrollbar gutter remains reserved per §3.17.
